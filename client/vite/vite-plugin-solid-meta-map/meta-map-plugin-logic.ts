...
import { cleanupDir, ensureDirExists, writeFile } from "./file-utils";
		...
		if (chunk.type === "chunk") {
			if (chunk.isEntry)
				entryChunkData.push({
					...
					// TODO
					css: Array.from(
						(chunk as any).viteMetadata?.importedCss || [],
					),
				});
		}
	}
...
export async function generateHtmlPages(
	...
	ensureDirExists(resolvedOutDir);
	...
	pages.forEach((page: Record<string, any>) => {
		// TODO: props type
		// TODO: don't think this way of rendering components will work with SolidJS
		// how to specify props when `renderToString`?
		let html = renderToString(() =>
			(PageTemplate as Component)({
				title: page.title,
		...
		generatedEntryChunks.forEach(chunk => {
			const isMatchingEntryPoint = chunk.modules.some(modulePath => {
				...
				const normalizedEntryPoint = page.bundleEntryPoint.replace(
					/\\/g,
					"/",
				);
		...
		// create .html file
		writeFile(filePath, `<!DOCTYPE html>${html}`);
	});

	// clean up the temporary dir using the callback method
	cleanupDir(tempBundleOutputDir);
}
